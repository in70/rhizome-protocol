...
/**
 * @title Compound's RHErc20 Contract
 * @notice RHTokens which wrap an EIP-20 underlying
 * @author Compound
 */
contract RHErc20 is RHToken, RHErc20Interface {
    /**
    ...
    function initialize(
        ...
        // RHToken initialize does the bulk of the work
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrow(address borrower, uint256 repayAmount, RHTokenInterface rhTokenCollateral)
        external
        returns (uint256)
    {
        (uint256 err,) = liquidateBorrowInternal(borrower, repayAmount, rhTokenCollateral);
        return err;
    }
