...
/**
 * @title Compound's RHErc20Delegator Contract
 * @notice RHTokens which wrap an EIP-20 underlying and delegate to an implementation
 * @author Compound
 */
contract RHErc20Delegator is RHTokenInterface, RHErc20Interface, CDelegatorInterface {
    /**
    ...
    function _setImplementation(address implementation_, bool allowResign, bytes memory becomeImplementationData) public {
        require(msg.sender == admin, "RHErc20Delegator::_setImplementation: Caller must be admin");
    ...
    function liquidateBorrow(address borrower, uint repayAmount, RHTokenInterface rhTokenCollateral) external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("liquidateBorrow(address,uint256,address)", borrower, repayAmount, rhTokenCollateral));
        return abi.decode(data, (uint));
    }
   ...
   /**
    ...
    function () external payable {
        require(msg.value == 0,"RHErc20Delegator:fallback: cannot send value to fallback");
